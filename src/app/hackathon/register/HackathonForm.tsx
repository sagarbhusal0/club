...
export default function HackathonForm({ categories, maxTeams = 0 }: { categories: string[]; maxTeams?: number }) {
  ...
  const { register, handleSubmit, trigger, watch, formState:{errors} } = useForm<FormData>({
    resolver: zodResolver(hackathonSchema) as never,
  ...
  const onSubmit = async (data: FormData) => {
    ...
    const res = await submitHackathonTeam({ ...data, members: data.members.map((m,i)=>({ ...m, isLeader: i===0 })) }, "client");
    setSubmitting(false);
  ...
  if (done) return (
    <Card className="text-center" style={{ animation:"scaleIn 320ms var(--ease-out) both" }}>
      ...
      <p className="mt-3 text-xs text-zinc-500 dark:text-zinc-400">Save your Team ID — you&apos;ll need it to check status and submit your final project.</p>
      <a href={`/hackathon/success?teamNumber=${encodeURIComponent(done.teamNumber)}`} className="mt-4 inline-flex min-h-11 items-center justify-center rounded-xl bg-indigo-600 px-6 py-2.5 text-sm font-semibold text-white">View success page →</a>
    </Card>
  );
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit, onInvalid)} className="space-y-4 sm:space-y-6" noValidate>
      ...
      <p className="text-xs font-medium uppercase tracking-wide text-zinc-500 dark:text-zinc-400">
        {step===1 && "Step 1 — Team & Project"}
        {step===2 && "Step 2 — Team Leader"}
        {step===3 && "Step 3 — Member 2"}
        {step===4 && "Step 4 — Member 3"}
        {step===5 && "Step 5 — Confirmation"}
      </p>
      ...
      {step===1 && (
        <Card className="space-y-4" style={{ animation:"fadeUp 220ms var(--ease-out) both" }}>
          ...
          <div><Label htmlFor="category">Category *</Label>
            <Select id="category" {...register("category")}><option value="">Select a category</option>{categories.map(c=><option key={c} value={c}>{c}</option>)}</Select>
            {e.category && <p className="mt-1.5 text-xs font-medium text-red-600 dark:text-red-400" role="alert">{e.category.message}</p>}
          </div>
          <div><Label htmlFor="description">Project description *</Label><Textarea id="description" rows={3} placeholder="What does your School Management project do?" {...register("description")} />{e.description && <p className="mt-1.5 text-xs font-medium text-red-600 dark:text-red-400" role="alert">{e.description.message}</p>}</div>
          ...
          <div><Label htmlFor="technologyStack">Technology stack <span className="font-normal text-zinc-400">— optional</span></Label><Input id="technologyStack" placeholder="Next.js, Python, ..." {...register("technologyStack")} /></div>
          <div><Label htmlFor="projectIdeaSummary">Project idea summary *</Label><Textarea id="projectIdeaSummary" rows={4} placeholder="What problem are you solving, for whom, and what makes your solution unique?" {...register("projectIdeaSummary")} />{ (e as never as { projectIdeaSummary?: { message:string } }).projectIdeaSummary && <p className="mt-1.5 text-xs font-medium text-red-600 dark:text-red-400" role="alert">{(e as never as { projectIdeaSummary: { message:string } }).projectIdeaSummary.message}</p>}<p className="mt-1 text-xs text-zinc-400">Reviewed by admins for uniqueness. If similar to another team, you may be asked to revise.</p></div>
          <Button type="button" onClick={()=>next(1)} className="w-full sm:w-auto">Continue →</Button>
        </Card>
      )}

      {step===2 && (
        <Card className="space-y-3" style={{ animation:"fadeUp 220ms var(--ease-out) both" }}>
          <h3 className="font-bold">Team Leader — only this member may use a laptop for the project</h3>
          <div className="grid gap-3 sm:grid-cols-2">
            <div><Label>Full name *</Label><Input {...register("members.0.fullName" as never)} />{ (errors as never as { members?: { message?:string }[] })?.members?.[0] && null }</div>
            <div><Label>Email *</Label><Input type="email" {...register("members.0.email" as never)} /></div>
            <div><Label>Phone *</Label><Input type="tel" {...register("members.0.phone" as never)} placeholder="98XXXXXXXX" /></div>
            <div><Label>Grade/Class *</Label><Input {...register("members.0.grade" as never)} /></div>
            <div><Label>Section *</Label><Input {...register("members.0.section" as never)} /></div>
            <div><Label>Student ID *</Label><Input {...register("members.0.studentId" as never)} /></div>
            ...
            <div><Label>GitHub URL <span className="font-normal text-zinc-400">— optional</span></Label><Input type="url" placeholder="https://github.com/..." {...register("members.0.githubUrl" as never)} /></div>
          </div>
          <div className="flex gap-2 pt-2"><Button type="button" onClick={()=>setStep(1)} className="bg-white text-zinc-900 ring-1 ring-inset ring-zinc-200 hover:bg-zinc-50 dark:bg-zinc-800 dark:text-zinc-100">← Back</Button><Button type="button" onClick={()=>next(2)}>Continue →</Button></div>
        </Card>
      )}
      {step===3 && (
        <Card className="space-y-3" style={{ animation:"fadeUp 220ms var(--ease-out) both" }}>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div><Label>Full name *</Label><Input {...register("members.1.fullName" as never)} /></div>
            <div><Label>Email *</Label><Input type="email" {...register("members.1.email" as never)} /></div>
            <div><Label>Phone *</Label><Input type="tel" {...register("members.1.phone" as never)} /></div>
            <div><Label>Grade/Class *</Label><Input {...register("members.1.grade" as never)} /></div>
            <div><Label>Section *</Label><Input {...register("members.1.section" as never)} /></div>
            <div><Label>Student ID *</Label><Input {...register("members.1.studentId" as never)} /></div>
            <div><Label>Role *</Label><Select {...register("members.1.role" as never)}>{MEMBER_ROLES.filter(r=>r!=="Team Leader").map(r=><option key={r} value={r}>{r}</option>)}</Select></div>
            <div><Label>GitHub URL <span className="font-normal text-zinc-400">— optional</span></Label><Input type="url" placeholder="https://github.com/..." {...register("members.1.githubUrl" as never)} /></div>
          </div>
          <div className="flex gap-2 pt-2"><Button type="button" onClick={()=>setStep(2)} className="bg-white text-zinc-900 ring-1 ring-inset ring-zinc-200 hover:bg-zinc-50 dark:bg-zinc-800 dark:text-zinc-100">← Back</Button><Button type="button" onClick={()=>next(3)}>Continue →</Button></div>
        </Card>
      )}
      {step===4 && (
        <Card className="space-y-3" style={{ animation:"fadeUp 220ms var(--ease-out) both" }}>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div><Label>Full name *</Label><Input {...register("members.2.fullName" as never)} /></div>
            <div><Label>Email *</Label><Input type="email" {...register("members.2.email" as never)} /></div>
            <div><Label>Phone *</Label><Input type="tel" {...register("members.2.phone" as never)} /></div>
            <div><Label>Grade/Class *</Label><Input {...register("members.2.grade" as never)} /></div>
            <div><Label>Section *</Label><Input {...register("members.2.section" as never)} /></div>
            <div><Label>Student ID *</Label><Input {...register("members.2.studentId" as never)} /></div>
            <div><Label>Role *</Label><Select {...register("members.2.role" as never)}>{MEMBER_ROLES.filter(r=>r!=="Team Leader").map(r=><option key={r} value={r}>{r}</option>)}</Select></div>
            <div><Label>GitHub URL <span className="font-normal text-zinc-400">— optional</span></Label><Input type="url" placeholder="https://github.com/..." {...register("members.2.githubUrl" as never)} /></div>
          </div>
          <div className="flex gap-2 pt-2"><Button type="button" onClick={()=>setStep(3)} className="bg-white text-zinc-900 ring-1 ring-inset ring-zinc-200 hover:bg-zinc-50 dark:bg-zinc-800 dark:text-zinc-100">← Back</Button><Button type="button" onClick={()=>next(4)}>Continue →</Button></div>
        </Card>
      )}
      ...
      {step===5 && (
        <Card className="space-y-4" style={{ animation:"fadeUp 220ms var(--ease-out) both" }}>
          ...
          <div className="rounded-xl bg-zinc-50 p-4 text-sm dark:bg-zinc-800/60">
            ...
            <p className="mt-2 text-xs text-zinc-500 dark:text-zinc-400">Idea: {(values.projectIdeaSummary || "—").slice(0,200)}</p>
            <div className="mt-3 grid gap-1 text-xs dark:text-zinc-300">
              {(values.members || []).map((m,i)=><p key={i}>{i===0?"👑 ":""}{m.fullName || "—"} · {m.email || "—"} · {m.studentId || "—"} · {m.role}</p>)}
            </div>
          </div>
          <label className="flex gap-3 rounded-xl border border-zinc-200 p-3 text-sm dark:border-zinc-700"><input type="checkbox" {...register("confirmInfo" as never)} className="mt-0.5" /> <span>I confirm that the information provided is correct.</span></label>
          ...
          <label className="flex gap-3 rounded-xl border border-zinc-200 p-3 text-sm dark:border-zinc-700"><input type="checkbox" {...register("confirmScratch" as never)} className="mt-0.5" /> <span>I confirm that this project will be created from scratch during the hackathon.</span></label>
          ...
          <div className="flex gap-2 pt-2"><Button type="button" onClick={()=>setStep(4)} className="bg-white text-zinc-900 ring-1 ring-inset ring-zinc-200 hover:bg-zinc-50 dark:bg-zinc-800 dark:text-zinc-100">← Back</Button><Button type="submit" disabled={submitting} className="flex-1 sm:flex-none">{submitting?"Registering...":"Register Team"}</Button></div>
        </Card>
      )}
