...
type Single = { teamName:string; projectTitle:string; status:string; teamNumber:string; adminNotes?: string | null };
...
export default function HackStatusPage() {
  ...
  return (
    <div className="bg-[#f8f7f5] dark:bg-zinc-950">
      ...
      <NoteModal open={!!modal} onClose={() => setModal(null)} title={modal?.teamName || ""} subtitle={modal ? `${modal.teamNumber} · ${modal.projectTitle}` : ""} status={modal?.status || ""} note={modal?.adminNotes ?? null} />
    </div>
  );
}
