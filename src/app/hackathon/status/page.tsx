...
function CardBtn({ a, onOpen }: { a: Single; onOpen: (a: Single) => void }) {
  return (
  ...
  return (
    <div className="bg-[#f8f7f5] dark:bg-zinc-950">
      ...
      <Card className="mt-6 space-y-4">
        ...
        {single && <div style={{ animation:"scaleIn 220ms var(--ease-out) both" }}><CardBtn a={single} onOpen={setModal} /></div>}
        {list && list.length>0 && (
          <div className="grid gap-3" style={{ animation:"scaleIn 220ms var(--ease-out) both" }}>
            ...
            {list.map(a=> <CardBtn key={a.teamNumber} a={a} onOpen={setModal} />)}
          </div>
        )}
      </Card>
      </div>
      <NoteModal open={!!modal} onClose={() => setModal(null)} title={modal?.teamName || ""} subtitle={modal ? `${modal.teamNumber} · ${modal.projectTitle} · Idea: ${modal.ideaStatus || "—"}${modal.isFinalSubmitted ? " · Final locked" : ""}` : ""} status={modal?.status || ""} note={modal?.adminNotes ?? modal?.ideaReviewNotes ?? null} />
    </div>
  );
}
